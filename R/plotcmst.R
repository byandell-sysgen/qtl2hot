...
##############################################################################
plot_bic_aic <- function(sim.out, my.cex, main="", penalty="BIC")
{
  graphics::par(mar=c(5,4.5,4,4)+0.1)
  r2s <- sim.out$R2s
  graphics::plot(r2s[,1],r2s[,2],xlab=expression(paste(R^{2}*(list(Y[1],Q)))),
...
##############################################################################
plot_par_cmst_joint <- function(sim.out, alpha, my.cex, 
...
##############################################################################
plot_par_cmst <- function(sim.out, alpha, my.cex, 
  main="", penalty=c("BIC","AIC"))
{
  graphics::par(mar=c(5,4.5,4,4)+0.1)
  r2s <- sim.out$R2s
...
##############################################################################
plot_non_par_cmst <- function(sim.out, alpha, my.cex, 
  main="", penalty=c("BIC","AIC"))
{
  graphics::par(mar=c(5,4.5,4,4)+0.1)
  r2s <- sim.out$R2s
